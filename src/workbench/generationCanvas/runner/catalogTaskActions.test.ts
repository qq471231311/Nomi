...
describe('buildCatalogTaskRequest — ComfyUI 自定义模型不吃旧档案 archetype', () => {
  it('本地 Comfy 首尾帧工作流即便残留 Dreamina archetype，也走 flat 首尾帧参数', () => {
    const node: GenerationCanvasNode = {
      ...
      prompt: '咖啡广告镜头',
      meta: {
        modelKey: 'comfy-wan2-2-mrvtg1hq',
        modelAlias: 'comfy-wan2-2-mrvtg1hq',
        modelVendor: 'comfyui-local',
        vendor: 'comfyui-local',
        modelLabel: 'wan2.2-首尾帧',
    ...
    const built = buildCatalogTaskRequest(node, {
      references: { firstFrameUrl: 'nomi-local://asset/first.png', lastFrameUrl: 'nomi-local://asset/last.png' },
    })
    ...
    expect(built.request.extras?.lastFrameUrl).toBe('nomi-local://asset/last.png')
  ...
  it('内置 comfyui-txt2img 这类非 comfy- 前缀本地模型也不吃旧 archetype', () => {
    const node: GenerationCanvasNode = {
      ...
      meta: {
        ...
        modelVendor: 'comfyui-local',
        vendor: 'comfyui-local',
        archetype: { id: 'dreamina-image', modeId: 'edit' },
      },
    }
    const built = buildCatalogTaskRequest(node, { references: { referenceImages: ['nomi-local://asset/ref.png'] } })
